...
final class ConsoleController: NSViewController, LocalProcessTerminalViewDelegate {
    ...
    override func loadView() {
        ...
        // `fm/cockpit-block-view-ssh-only`: mirrors the theme observer above -
        ...
        // "flip ALL currently open tabs immediately" spec. Only ever visibly
        // affects a tab where `TabModel.supportsBlockView` is true (SSH host
        // pages) - see `updateTabViewVisibility`.
    ...
    private func buildTabBar() {
        ...
        // view-ssh-only`) are both dedicated-host-page-only affordances - the
        // shared Firstmate console has no single host cluster to investigate,
        // and its Shell/Mirror tabs never support block view (see
        // `TabModel.supportsBlockView` for why - a bug there took down the
        ...
        if !isFirstmateConsole {
            toolViews.append(blockViewButton)
        }
    ...
    private func addTab(launch: TabLaunch, name: String, select: Bool, accentHex: String? = nil, isOneShotCommand: Bool = false) -> TabModel {
        ...
        // `fm/cockpit-block-view-ssh-only`: an SSH tab (never `.shell`/
        // `.mirror` - see `TabModel.supportsBlockView`) gets a block tracker
        // attached to its `Terminal` and a sibling rendering view, both
        // created up front so they're ready the instant the global toggle
        // (or a later select) turns block view on for this tab - never
    ...
    /// `fm/cockpit-block-view-ssh-only`: best-effort, same timing convention
    ...
    /// for the remote SSH session (authentication + remote prompt) to be
    /// sitting at a real prompt. A no-op for `.shell`/`.mirror` (see
    /// `TabModel.supportsBlockView`) and for a one-shot provisioning command
    /// (`isOneShotCommand`) - neither is an interactive prompt cycle this
    /// hook has anything to attach to.
    ...
    // MARK: Block view (`fm/cockpit-block-view-ssh-only`)
    ...
    /// accumulating blocks long before block view is ever shown). A tab
    /// where `supportsBlockView` is false (the Firstmate console's Shell/
    /// Mirror tabs, a plain ad-hoc local shell tab) has no `blockContainer`
    /// at all, so it always shows raw `terminal` regardless of the global
    /// flag.
    ...
    /// A no-op on the shared Firstmate console, which never adds this button
    /// to its toolbar at all (see `buildTabBar`).
    private func updateBlockViewButton() {
        guard !isFirstmateConsole else { return }
