...
final class CockpitTerminalView: LocalProcessTerminalView {
    ...
    /// (when present - SSH tabs only, see `TabModel.supportsBlockView`) to
    /// refresh its currently-running block's output text, so block view
    /// streams live instead of only updating once a command finishes and its
    /// closing OSC 133 marker fires.
