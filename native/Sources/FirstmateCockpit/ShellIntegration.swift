...
//
// `fm/cockpit-block-view-terminal`: the shell-integration hook that makes
...
// bash/zsh snippet, injected into a tab exactly the way `Host.startupSnippetID`
// already injects text into a freshly-connected tab (`ConsoleController.
// runStartupSnippet`) - same mechanism, not a new one - that makes bash/zsh
// emit OSC 133 "semantic prompt" escape sequences (the same convention VS
// Code's and iTerm2's shell integration use) around every prompt cycle.
//
...
// that DEBUG-trap version was built and tested live in this task (see the PR
// description) and it mis-fired: bash's DEBUG trap also fires for each
// *sub-statement* of `PROMPT_COMMAND` itself when `PROMPT_COMMAND` is a
// compound `stmt1; stmt2` string (confirmed live: it double-fired on the
...
// (see the PR description for the transcripts) before writing any Swift.
//
...
// this machine. No `PS0`/`preexec_functions`-only feature is required.
enum ShellIntegration {

    /// The literal shell text sent into a tab once its process starts (mirrors
    /// `ConsoleController.runStartupSnippet`'s injection mechanism). Self-
    /// detects bash vs. zsh at `source`-time and installs the matching hook;
    /// a shell that is neither (e.g. `sh`, `fish`, `csh`) silently does
