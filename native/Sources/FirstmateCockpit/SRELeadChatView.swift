...
final class SRELeadChatView: NSView, NSTextFieldDelegate {
    ...
    /// `SRELeadMarkdownView` (`fm/cockpit-block-view-error-explain` extracted
    /// the per-block rendering out of this file so the block-view "Explain
    /// this" action could reuse it too - the visual output is unchanged).
