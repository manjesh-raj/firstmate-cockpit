...
// blocks (paragraph/bulletList/codeBlock/callout), each rendered as its own
// small AppKit view. `fm/cockpit-block-view-ssh-only` (originally PR #80)
// extracted that per-block rendering into `SRELeadMarkdownView.swift` so
// block view's "Explain this" action could reuse it - `assistantBlock(for:)`
// below just wraps one in this pane's own message-bubble chrome. User/
// status/error messages are unchanged (plain text - a captain's own typed
// question, or a short internal status/error string, neither of which
// carries the persona's markdown convention).
...
final class SRELeadChatView: NSView, NSTextFieldDelegate {
    ...
    /// `SRELeadMarkdownView` (`fm/cockpit-block-view-ssh-only` extracted the
    /// per-block rendering out of this file so the block-view "Explain this"
    /// action could reuse it too - the visual output is unchanged).
    private func assistantBlock(for text: String) -> NSView {
        ...
        NSLayoutConstraint.activate([
            markdownView.leadingAnchor.constraint(equalTo: container.leadingAnchor, constant: 10),
            markdownView.trailingAnchor.constraint(equalTo: container.trailingAnchor, constant: -10),
            markdownView.topAnchor.constraint(equalTo: container.topAnchor, constant: 8),
            markdownView.bottomAnchor.constraint(equalTo: container.bottomAnchor, constant: -8),
        ])
        return container
    }
