...
//
// `fm/cockpit-block-view-error-explain`: the "Explain this" block action -
// a single, captain-triggered `claude -p` call that explains a failed
// command's output. This is explicitly NOT SRE Lead (`SRELead.swift`): no
// MCP server, no kubectl access, no session/`--resume` threading across
...
// a second way to shell out to `claude`.
//
