...
// `json`, not `stream-json`). Extracted here (`fm/cockpit-block-view-error-
// explain`) so the new "Explain this" block action can shell out to `claude`
// the same way instead of hand-rolling a second copy of this - callers own
// their own argv and JSON interpretation; this only runs the process and
// hands back the parsed JSON object (or a failure message) on the main
// thread.
