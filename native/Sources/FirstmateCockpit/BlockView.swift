...
//
// `fm/cockpit-block-view-ssh-only`: the native, parsed rendering mode -
...
// every other modern-UI page already does. Scoped to SSH host page tabs only
// (`TabModel.supportsBlockView`) - see AGENTS.md's block-view section for the
// scope narrowing and the crash this redo fixed.
//
// Adds the "Explain this \u{2726}" action on a failed block (see
// `BlockExplain.swift`) and its response panel, rendered via
// `SRELeadMarkdownView` - the same renderer the SRE Lead pane uses, not a
// second styling. This is explicitly not SRE Lead: one captain-triggered
// `claude -p` call, no tool access, no session.
//
// **Crash history (root-caused, not guessed):** the original PR #79/#80
// shipped `BlockContainerView.render(_:)` activating
...
// Shell tab used to be `supportsBlockView` too and auto-opens at launch, the
// shell-integration hook's first OSC 133 marker fired almost immediately,
// opening the first block and crashing the app on every single launch. The
// fix below is simply the correct order: add the row as an arranged subview
// first, activate the width constraint second. A real-view-hierarchy self
// test (`BlockViewHierarchySelfTest.swift`) now mounts this exact view in a
// live `NSWindow` and drives a full open-block/close-block cycle through
// real Auto Layout, specifically to catch a regression of this class of bug
// - the original self-tests only exercised `TerminalBlockTracker`'s parsing
// logic and never mounted `BlockContainerView` at all.
...
/// `content`, shown or hidden by `ConsoleController.applyBlockViewVisibility`
/// without ever touching the underlying process. Rebuilds its rows wholesale
/// on every `TerminalBlockTracker.onChange` - the same "small enough list,
/// just redraw it" convention `BootstrapController`'s dynamic sections
/// already use.
final class BlockContainerView: NSView {
    ...
    func render(_ blocks: [TerminalBlock]) {
        ...
        for block in blocks {
            ...
            // `stack.addArrangedSubview(row)` MUST come before activating a
            // constraint that references `stack` - this order was reversed
            // in the original PR #79/#80 code and is exactly what crashed
            // the app on every launch (see this file's header). Do not
